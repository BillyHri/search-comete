...
Usage:
  # Recommended first run — arXiv, no ES needed, ~10 min
  python -m pipeline.run --limit 50 --use-arxiv --skip-index

  # Full arXiv run — all clusters, ~30 min
  python -m pipeline.run --limit 200 --use-arxiv --skip-index

  # Resume an interrupted fetch (picks up from last completed cluster)
  python -m pipeline.run --limit 50 --use-arxiv --skip-index --resume

  # Skip fetch entirely, re-run embed + UMAP + export from cache
  python -m pipeline.run --skip-fetch --skip-index

  # With Elasticsearch running
  python -m pipeline.run --limit 200 --use-arxiv

Rate limits:
  - arXiv:            free, no key, ~1.5s between requests  ← recommended
  - Semantic Scholar: free but strict. Set SS_API_KEY env var for 10x higher limits.
...
PROGRESS_JSON  = os.path.join(CACHE_DIR, "progress.json")   # per-topic progress for --resume
...
def main():
    ...
    if args.use_ss:
        print("⚠  Using Semantic Scholar — consider --use-arxiv to avoid rate limits")
        if not os.getenv("SS_API_KEY"):
            print("   Tip: set OPENALEX_EMAIL=your@email.com for polite pool priority")
            print("        then: export SS_API_KEY=your_key_here")
    ...
    if args.skip_fetch and os.path.exists(PAPERS_JSON):
        ...
        fetch_fn = fetch_openalex if not args.use_arxiv and not args.use_ss else (fetch_arxiv if args.use_arxiv else fetch_semantic_scholar)
        print(f"\n[1/5] Fetching papers (limit={args.limit} per topic, source={source})…")

        # --resume: load any previously completed topics from progress file
        completed = {}
        ...
        for i, (query, cid, clabel, ccolor) in enumerate(TOPICS):
            ...
            if topic_key in completed:
                print(f"  [{i+1:02d}/{len(TOPICS)}] [{cid.upper()}] skipping (already done) — {len(completed[topic_key]['papers'])} papers")
                continue

            print(f"\n  [{i+1:02d}/{len(TOPICS)}] [{cid.upper()}] {query[:55]}…")
            fetched = fetch_fn(query, limit=args.limit)
            print(f"  → {len(fetched)} papers")

            cl = {"id": cid, "label": clabel, "color": ccolor}
            topic_papers   = fetched
            topic_clusters = [cl] * len(fetched)

            papers.extend(topic_papers)
            cluster_infos.extend(topic_clusters)

            # Save progress after each topic so --resume works on crash
            completed[topic_key] = {
                "papers":   topic_papers,
    ...
    # Show final cluster breakdown
    from collections import Counter
    counts = Counter(s["cluster"] for s in stars)
    print("\n  Cluster breakdown:")
    for cid, n in sorted(counts.items()):
        print(f"    {cid:8s}: {n} stars")
    ...
    if not args.skip_index:
        ...
        try:
            ...
            print("  stars.json was still exported — frontend can use it directly")
    else:
        print("\n[5/5] Skipping Elasticsearch (--skip-index)")

    print(f"\n✓ Pipeline complete — {len(docs)} papers processed")
    print(f"\nNext steps:")
    print(f"  cp pipeline/stars.json frontend/public/stars.json")
    print(f"  Then hard-refresh the browser (Ctrl+Shift+R)")
